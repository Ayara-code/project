...
public:
	...
	*/
	explicit ThreadSafeImageQueue(size_t max_size = 10,
		bool shutdown_on_destruct = true)
		: max_size_(max_size)
		, shutdown_flag_(false)
		, shutdown_on_destructor_(shutdown_on_destruct)
	{
		if (max_size == 0)
		{
	...
	*/
	ThreadSafeImageQueue(ThreadSafeImageQueue&& other) noexcept
	{
		...
		shutdown_flag_ = other.shutdown_flag_;
   ...
   */
	bool pop(ImageType& img, int timeout_ms = 0)
	{
		...
		if (queue_.empty()) //不检查是否队列关闭，保证超时等待后能处理完剩余的图像数据
		{
			return false;
		}
...
private:
	...
	std::atomic<bool> shutdown_flag_;	// 关闭标志位
	...
	*/
	...
	{
		// 即使 shutdown 了，如果队列里还有数据，也不能直接返回 false，得让消费者取完
		if (!queue_.empty())
		{
			return true;
		}
		// 只有当队列空了，且已经设置了关闭标志，才真正返回 false 结束消费者
		if (shutdown_flag_)
		{
			return false;
		}

		auto predicate = [this]() { return !queue_.empty() || shutdown_flag_; };

		if (timeout_ms <= 0)
		{
			not_empty_cond_.wait(lock, predicate);
			// 唤醒后：只要队列不为空，就应该允许消费！
			return !queue_.empty();
		}
		else
		{
			not_empty_cond_.wait_for(lock, std::chrono::milliseconds(timeout_ms), predicate);
			// 唤醒或超时后：只要队列不为空，就应该允许消费！
			return !queue_.empty();
		}
	}
